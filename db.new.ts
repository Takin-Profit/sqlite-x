...
export class DB {
	...
	sql<P extends DataRow, R = unknown>(
		...
		let builder = new Sql<P>(strings, params, {} as any, this.#formatConfig)
		return createXStatementSync<P, R>({
			build: finalParams => {
				...
				const { sql, namedParams, hasJsonColumns } = builder.prepare()
				const stmt = this.prepareStatement(sql)

				return { stmt, namedParams, hasJsonColumns }
			},
			prepare: stmt => this.prepareStatement(stmt),
			sql: builder,
		})
	}
